...
- Under the full Windows suite, the serialized PTY queue can legitimately exceed 15 seconds. The PTY session lock budget is therefore 30000 ms, and slow async tests such as `tui-lazy-loading` declare explicit larger per-test budgets instead of relying on Vitest's default 5-second timeout.
